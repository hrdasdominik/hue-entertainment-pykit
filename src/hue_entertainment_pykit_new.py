...
class HueEntertainmentPyKit:
    def __init__(self):
        setup_logging(level=logging.DEBUG, max_file_size=1024 * 1024 * 5, backup_count=3)
    ...
    def status_stream(self):
        if self._streaming is None:
            raise Exception("Stream status is none due to streaming being None")

        self._streaming.is_stream_active()

    def stop_stream(self):
        if self._streaming is None:
            raise Exception("Stream not stopped due to streaming is None")
    ...
    def set_entertainment_configuration(self, entertainment_configuration: EntertainmentConfiguration):
        ...
        self._streaming = StreamingService(
            self._entertainment_configuration, self._entertainment_configuration_repository, self._dtls
        )
