class ColorValidatorUtil:
    ...
    def is_user_input_valid(user_input):
        ...
        if not ColorValidatorUtil.is_valid_rgb(color) and not ColorValidatorUtil.is_valid_xyb(color):
            raise ValueError(
                "Invalid input: values must be a valid rgb8 (0 - 255) or xyb (0.0 - 1.0)"
            )

    @staticmethod
    def is_valid_rgb(rgb8: tuple[int, int, int]) -> bool:
    ...
    @staticmethod
    def is_valid_xyb(xyb: tuple[float, float, float]) -> bool:
